!function(){
	...
	var rcolors = ["#7fc97f","#beaed4","#fdc086","#ffff99","#386cb0","#f0027f","#bf5b17","#666666"];
...
bP.draw2 = function(data,svg){
	...
	//----------------------------------Patterns and HTML Elements----------------------------------------------//
			...
			document.getElementById("align2").onclick = function() { align2(data,visData)};
...
function addText(dat){
	...
	function starburstP(data,visData){
		...
		star(data,mapp(x1),visData);
	}

	function slideP(data,visData){
		var x1 = prompt("Enter the value from which you wish to observe the slide");
		console.log("For Slide Pattern");
		slid(data,mapp(x1),visData);
	}
	...
	function star(data,x,visData){
		...
		data.forEach(function(biP,s){
			...
			d3.select("#"+biP.id).select(".edges").selectAll(".edge")
			.filter(function(d){
				return !(d.h1 !=0 && d.h2 !=0 && d.key1 != x && d.key2 ==x);
			})
			.style("opacity",0);
	...
	function align33(data,visData){
		...
		data.forEach(function(biP,s){
			...
			if(cou==4){
				d3.select("#"+biP.id).select(".edges2").selectAll(".edge2")
				.filter(function(d){
					return (d.h1 !=0 && d.h2 !=0 &&(d.key1 == d.key2) && (d.key2==d.key3) );
				})
				.style("opacity",0.9);

				d3.select("#"+biP.id).select(".edges2").selectAll(".edge2")
				.filter(function(d){
					return !((d.key1 == d.key2) && (d.key2==d.key3));
				})
				.style("opacity",0);
			}
			d3.select(".chart"+s).attr("opacity",0);	//Hiding the bar graph of Switch function
	...
	function align2(data,visData){
		...
		data.forEach(function(biP,s){
			...
			d3.select("#"+biP.id).select(".edges").selectAll(".edge")
			.filter(function(d){
				return (d.h1 != 0 && d.h2 != 0 && d.key1 != d.key2 );
			})
			.style("opacity",0);
	...
	function switchF(data,svg,visData){
		...
		data.forEach(function(biP,s){
			...
			bar.append("text")
				.attr("x",function(d) { return width1 + 15; })
				.attr("y",barHeight/2)
				.attr("dy", ".35em")
    			.text(function(d,i) { return remapp((transpose(initial))[0][i]); });
	...
	function returnP(data,visData){
		...
		data.forEach(function(biP,s){
			...
			if(s==0){
				d3.select("#"+biP.id).select(".edges2").selectAll(".edge2")
				.filter(function(d){
					return (d.h1!=0 && d.h2!=0 && (d.key1 == d.key3) && (d.key1 !=d.key2) );
				})
				.style("opacity",0.9)
				.style("fill",function(d){ return colors[d.key1];});
				...
				d3.select("#"+biP.id).select(".edges2").selectAll(".edge2")
				.filter(function(d){
					return (d.h1!=0 && d.h2!=0 && (d.key2 == d.key3) && (d.key1 !=d.key2) );
				})
				.style("opacity",0.9)
				.style("fill",function(d){ return colors[d.key3];});
			...
			var temp,i,j;
			console.log("For Return Pattern");
			for(i=0;i<nArray.length;i++){
				if(nArray[i][0]== nArray[i][2] && nArray[i][0] != nArray[i][1]) {
					console.log(remapp(nArray[i][0])+ "->" + remapp(nArray[i][1]) + "->" + remapp(nArray[i][2]) + " count = "+ nArray[i].deepSortAlpha);
				}
			}
	}
